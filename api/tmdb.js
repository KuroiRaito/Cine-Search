...
/**
 * How long an answer stays good, by what was asked.
 *
 * A film's credits do not change between one visitor and the next; the
 * trending list does. One number for both meant re-asking TMDB for Inception
 * every sixty seconds across every device, to keep a feed fresh that the film
 * page never reads. Entities keep for an hour at the edge and half an hour
 * here; feeds and searches keep for a minute, as before.
 */
function isEntity(path) {
  const p = String(path).replace(/^\/+/, '');
  return /^(movie|tv|person|collection)\/\d+/.test(p) || /^genre\//.test(p);
}
const ttlFor = (path) => (isEntity(path) ? 30 * 60 * 1000 : 60 * 1000);
const cacheControlFor = (path) => (isEntity(path)
...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (status >= 200 && status < 300) {
      setCached(cacheKey, data, ttlFor(path));
      response.setHeader('Cache-Control', cacheControlFor(path));
    }
